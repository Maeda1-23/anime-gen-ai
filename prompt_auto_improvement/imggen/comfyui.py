...
class ComfyUIGenerator(ImageGenerator):
    ...
    def generate_image(
        ...
        workflow = self._patch_workflow(
            workflow, self.config.positive_prompt_node_id,
            self.config.positive_prompt_input_key, positive_prompt
        )

        if negative_prompt:
            workflow = self._patch_workflow(
                workflow, self.config.negative_prompt_node_id,
                self.config.negative_prompt_input_key, negative_prompt
            )

        if seed is not None:
            workflow = self._patch_workflow(
                workflow, self.config.seed_node_id,
        ...
        try:
            ...
            while True:
                ...
                if evt.get("type") in ("execution_success", "executed"):
                    ...
                    if evt_data.get("prompt_id") == prompt_id:
                        print("画像生成完了")
                        break
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            with urllib.request.urlopen(url, timeout=5) as resp:
                ...
                return "system" in data or "queue_remaining" in data
        except Exception as e:
